...
var tinynlp = function () {
    ...
    h.prototype.log = function (a) { if (loggingOn) { console.log("-------------------"); console.log("Column: " + a); console.log("-------------------"); for (var c in this.chart[a]) console.log(this.chart[a][c].toString()) } }; g.prototype.complete = function () { return this.dot >= this.rhs.length }; g.prototype.toString = function () {
        var a = []; a.push("(id: " + this.id + ")"); a.push(this.lhs); a.push("\u2192"); for (var c = 0; c < this.rhs.length; c++)c == this.dot && a.push("\u2022"), a.push(this.rhs[c]); this.complete() && a.push("\u2022"); a.push("[" +
            this.left + ", " + this.right + "]"); a.push(JSON.stringify(this.ref)); return a.join(" ")
    }; g.prototype.expectedNonTerminal = function (a) { return null !== a.getRightHandSides(this.rhs[this.dot]) ? !0 : !1 }; g.prototype.setId = function (a) { this.id = a }; g.prototype.getId = function () { return this.id }; g.prototype.equals = function (a) { return this.lhs === a.lhs && this.dot === a.dot && this.left === a.left && this.right === a.right && JSON.stringify(this.rhs) === JSON.stringify(a.rhs) ? !0 : !1 }; g.prototype.getRefsToChidStates = function () { return this.ref };
    g.prototype.appendRefsToChidStates = function (a) { for (var c = !1, b = 0; b < a.length; b++)if (a[b]) for (var d in a[b]) this.ref[b][d] != a[b][d] && (c = !0), this.ref[b][d] = a[b][d]; return c }; g.prototype.predictor = function (a, c) { var b = this.rhs[this.dot], d = a.getRightHandSides(b), e = !1, f; for (f in d) { for (var k = d[f], h = 0; k && h < k.length && a.isEpsilonProduction(k[h]);)h++; k = new g(b, k, h, this.right, this.right); e |= c.addToChart(k, this.right) } return e }; g.prototype.scanner = function (a, c, b) {
        var d = this.rhs[this.dot], e = !1; (a = b ? a.terminalSymbols(b) :
            []) || (a = []); a.push(b); for (var f in a) if (d == a[f]) { b = new g(d, [b], 1, this.right, this.right + 1); e |= c.addToChart(b, this.right + 1); break } return e
    }; g.prototype.completer = function (a, c) {
        var b = !1, d = c.getStatesInColumn(this.left), e; for (e in d) {
            var f = d[e]; if (f.rhs[f.dot] == this.lhs) {
                for (var k = f.dot + 1; f.rhs && k < f.rhs.length && a.isEpsilonProduction(f.rhs[k]);)k++; k = new g(f.lhs, f.rhs, k, f.left, this.right); k.appendRefsToChidStates(f.ref); var h = Array(f.rhs.length); h[f.dot] = {}; h[f.dot][this.id] = this; k.appendRefsToChidStates(h);
